...
fn wavelet_matrix_new(data: *mut Box<[u32]>) -> *mut WM {
    ...
    allocate(WaveletMatrix::<DenseBitVec>::new(
        data.into(),
        max_symbol,
        Default::default(),
        None,
...
fn wavelet_matrix_counts(
    ...
    let mut counts = wm.counts(&[range_lo..range_hi], symbol_extent_lo..=symbol_extent_hi);
    ...
    let mut ends = vec![];
    for x in results {
        symbols.push(x.v.symbol);
        starts.push(x.v.start);
        ends.push(x.v.end);
    }
